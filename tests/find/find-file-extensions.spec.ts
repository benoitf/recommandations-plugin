...
describe('Test FindFile implementation', () => {
  ...
  test('multiple end event', async () => {
    ...
    const findFileExtensions = container.get(FindFileExtensions);
    const findPath = path.join(__dirname, '..', '_data', 'find');
    const fileExtensions = await findFileExtensions.findInFolder(findPath, 500);
    await sleep(1000);
    expect(fileExtensions).toBeDefined();
    expect(fileExtensions).toBeDefined();
    expect(fileExtensions.length).toBe(4);
  ...
  test('stream error', async () => {
    (globby as any).__setStreamError('invalid data');
    const findFileExtensions = container.get(FindFileExtensions);
    const findPath = path.join(__dirname, '..', '_data', 'find');
    return expect(
      findFileExtensions.findInFolder(findPath, FindFileExtensions.DEFAULT_SCAN_TIME_PER_WORKSPACE_FOLDER)
    ).rejects.toEqual(new Error('invalid data'));
  });
});
